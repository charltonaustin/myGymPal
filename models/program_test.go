...
func TestCreateProgram_Success(t *testing.T) {
	...
	p, err := CreateProgram(u.ID, "Hypertrophy Block", testStartDate, 4, 8)
	require.NoError(t, err)
...
func TestCreateProgram_EmptyName(t *testing.T) {
	...
	_, err := CreateProgram(u.ID, "", testStartDate, 4, 8)
	assert.Error(t, err)
}
...
func TestCreateProgram_ZeroPhases(t *testing.T) {
	...
	_, err := CreateProgram(u.ID, "Bad Program", testStartDate, 0, 8)
	assert.Error(t, err)
}
...
func TestCreateProgram_NegativePhases(t *testing.T) {
	...
	_, err := CreateProgram(u.ID, "Bad Program", testStartDate, -1, 8)
	assert.Error(t, err)
}
...
func TestCreateProgram_ZeroWeeksPerPhase(t *testing.T) {
	...
	_, err := CreateProgram(u.ID, "Bad Program", testStartDate, 4, 0)
	assert.Error(t, err)
}
...
func TestCreateProgram_NegativeWeeksPerPhase(t *testing.T) {
	...
	_, err := CreateProgram(u.ID, "Bad Program", testStartDate, 4, -1)
	assert.Error(t, err)
}
...
func TestGetProgramsByUserID(t *testing.T) {
	...
	p1, err := CreateProgram(u.ID, "Program A", testStartDate, 3, 8)
	require.NoError(t, err)
	p2, err := CreateProgram(u.ID, "Program B", testStartDate.AddDate(0, 0, 7), 2, 8)
	require.NoError(t, err)
	t.Cleanup(func() {
...
func TestGetProgramsByUserID_OtherUsersNotIncluded(t *testing.T) {
	...
	p, err := CreateProgram(u1.ID, "U1 Program", testStartDate, 2, 8)
	require.NoError(t, err)
...
func TestGetProgramByID_Found(t *testing.T) {
	...
	p, err := CreateProgram(u.ID, "My Program", testStartDate, 5, 8)
	require.NoError(t, err)
	t.Cleanup(func() { DeleteProgram(p.ID, u.ID) })
...
func TestDeleteProgram(t *testing.T) {
	...
	p, err := CreateProgram(u.ID, "To Delete", testStartDate, 1, 8)
	require.NoError(t, err)
...
func TestDeleteProgram_WrongUser(t *testing.T) {
	...
	p, err := CreateProgram(u1.ID, "Protected", testStartDate, 2, 8)
	require.NoError(t, err)
	t.Cleanup(func() { DeleteProgram(p.ID, u1.ID) })
