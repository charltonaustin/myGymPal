...
| exercise   | Full CRUD for the user's exercise library at /exercises; three AJAX POST endpoints for live goal-weight, goal-reps, and goal-seconds updates from within a session. GET /exercises/history renders a Chart.js line chart where users can compare multiple exercises over time; GET /exercises/history/data returns JSON series data (max weight or max reps per session per exercise). | [128w](controllers/exercise.128w.md) · [256w](controllers/exercise.256w.md)   |
