...
| session    | Most complex controller; full workout-session lifecycle at /programs/:id/sessions and /sessions/:id; logs sets and cardio; auto-promotes goal weight after 3rd successful set. A floating rest-timer control on the session page starts the post-set timer and edits the default rest length in 15s steps; POST /sessions/:id/rest persists that length to the session's phase (JSON response). | [128w](controllers/session.128w.md) · [256w](controllers/session.256w.md)     |
