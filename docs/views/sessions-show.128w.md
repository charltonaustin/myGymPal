...
`localStorage`. A global lb/kg toggle converts all exercises at once; each weighted exercise card also has its own
per-exercise lb/kg toggle that persists the preference to the exercise library via
`POST /sessions/:id/exercises/:eid/unit`. Exercise autocomplete fills type radio from `.ExerciseLibraryJSON`.
