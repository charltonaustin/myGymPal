...
deleting cardio activities, deleting exercises, deleting sets (AJAX), reordering exercises (AJAX), and updating a single
exercise's display unit (`UpdateExerciseUnit`, AJAX). After the 3rd set at or above goal reps and goal weight, the
exercise library goal weight is automatically promoted via `Exercises.UpdateGoalWeight`.
