...
| session_exercise | An exercise entry within a session with goal weight/reps/seconds, block assignment, sort order, and a `linked_to_next` superset flag (`UpdateLinkedToNext`); paired with SessionSet and CardioLog sub-records.        | [128w](models/session_exercise.128w.md) |
